...
public class ParserService {
	...
	public WiktionaryEntryPageParseResult parseWiktionaryEntryPage(String page, TemplateService templateService) throws ParseException {
		if (page == null)
		...
		if (parser.parse(new SourceString(correctedPage)))
			...
			erroneousEntriesLogger.error(parser.semantics().getLemma() != null ? parser.semantics().getLemma() : "unknown");
	...
	public TemplateDefinitionPageParseResult parseTemplateDefinitionPage(String page) throws ParseException {
		...
		if (parser.parse(new SourceString(page)))
			return new TemplateDefinitionPageParseResult(parser.semantics().getTemplateDefinition(), parser.semantics().getTemplates());
		else
			throw new ParseException();
	}
	...
	private class ParseWiktionaryEntryPageTask implements Runnable {
		...
		public void run() {
			try {
				...
				consumer.accept(parseWiktionaryEntryPage(pageDocument.getText(), templateService));
